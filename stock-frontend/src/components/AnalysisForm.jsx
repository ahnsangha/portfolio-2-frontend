...
export default function AnalysisForm({ onStart }) {
  ...
  useEffect(() => {
    ...
    if (windowSize < 20 || windowSize > 250) newErrors.window = 'Window 값은 20에서 250 사이여야 합니다.';

    //  최대 선택 개수를 100개에서 30개로 변경합니다.
    if (selectedTickers.length < 2) newErrors.tickers = '최소 2개 이상의 종목을 선택해주세요.';
    else if (selectedTickers.length > 30) newErrors.tickers = '최대 30개까지 선택 가능합니다.';

    setErrors(newErrors);
  }, [startDate, endDate, windowSize, selectedTickers]);
  ...
  return (
    <div className="">
      ...
      <button
        ...
        className={`group relative w-full md:w-auto px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-xl hover:shadow-2xl hover:shadow-purple-500/25 transition-all duration-300 ${isLoading || hasErrors ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105'}`}
      >
        {isLoading ? (
